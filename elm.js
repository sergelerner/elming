...
Elm.Main.make = function (_elm) {
   ...
   var view = F2(function (address,model) {
      return A2($Html.div,
      _U.list([]),
      _U.list([A2($Html.button,_U.list([A2($Html$Events.onClick,address,Increase)]),_U.list([$Html.text("+")]))
              ,A2($Html.span,_U.list([]),_U.list([$Html.text($Basics.toString(model))]))
              ,A2($Html.button,_U.list([A2($Html$Events.onClick,address,Decrease)]),_U.list([$Html.text("-")]))]));
   });
   var model = 0;
   var main = $StartApp$Simple.start({model: model,view: view,update: update});
   return _elm.Main.values = {_op: _op,model: model,Increase: Increase,Decrease: Decrease,update: update,view: view,main: main};
};
